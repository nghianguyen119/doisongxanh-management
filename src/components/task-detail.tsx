...
export async function TaskDetail({ id }: { id: string }) {
  ...
  const assigneeItems = employees.map((e) => ({
    value: e.id,
    label: e.zaloUserId ? e.name : `${e.name} (chưa kết nối Zalo)`,
  }));
  const priorityItems = taskPriority.enumValues.map((p) => ({
    value: p,
          ...
          <Card className="gap-3 p-5">
            ...
            >
              ...
              <Field>
                ...
                <Select name="assigneeId" required items={assigneeItems}>
